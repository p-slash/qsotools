...
if __name__ == '__main__':
    ...
    parser.add_argument("FileNameList", help="List of relative path to spectra.")
    parser.add_argument("OutputDir", help="Output directory.")

    parser.add_argument("--reverse", help="Convert flux fluctuations back to flux. If sigma passed, it does not convert to flux.", action="store_true")
    
    parser.add_argument("--mean-flux-file", "-mff", help="Table for mean flux.")
    parser.add_argument("--mean-flux-fg08", "-fg08", help="Use FG 08 mean flux.", action="store_true")
    parser.add_argument("--analytic-ln-flux", "-aln", help="Use analytic lognormal flux", action="store_true")
    parser.add_argument("--without-z-evolution", "-noz", help="Turn off redshift evolution by computing mean flux at median z.", action="store_true")
    parser.add_argument("--chunk-mean", "-ch", help="Use mean flux of the chunk.", action="store_true")
    parser.add_argument("--const-mean-flux", "-cmf", help="Constant mean flux.", type=float)
    
    parser.add_argument("--seed", type=int, default=2342)
    parser.add_argument("--sigma-eta0", "-s0", help="Continuum noise amplitude in reverse. Std dev of eta 0. Default: %(default)s", type=float, default=0)
    parser.add_argument("--sigma-eta1", "-s1", help="Continuum noise slope in reverse. Std dev of eta 1. Default: %(default)s A", type=float, default=0)
    ...
    if args.mean_flux_file:
        ...
        print("Mean flux is constant across redshift: ", args.const_mean_flux, flush=True)
        mean_flux_function = lambda z: args.const_mean_flux
    else:
    ...
    if (args.sigma_eta0 > 0 or args.sigma_eta1 > 0) and args.reverse:
        print("Adding continuum noise with sigma-eta-0=%.2f and sigma-eta-1=%.2f" %(args.sigma_eta0, args.sigma_eta1), flush=True)
    ...
    for fl in file_list:
        ...
        spectrum_z = np.array(spectrum.wave, dtype=np.double)  / lm.LYA_WAVELENGTH - 1
        ...
        if args.reverse:
            ...
            if args.sigma_eta0 > 0 or args.sigma_eta1 > 0:
                ...
                spectrum.flux  = (1+np.array(spectrum.flux, dtype=np.double)) * mean_flux_function_array 
                spectrum.error = np.array(spectrum.error, dtype=np.double) * mean_flux_function_array
        else:
            spectrum.flux  = np.array(spectrum.flux, dtype=np.double)  / mean_flux_function_array - 1
            spectrum.error = np.array(spectrum.error, dtype=np.double) / mean_flux_function_array

        try:
            spectrum.saveas(ospath_join(args.OutputDir, fl.rstrip()))
        except:
            print("Problem saving ", ospath_join(args.OutputDir, fl.rstrip()), flush=True)
