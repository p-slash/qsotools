...
if __name__ == '__main__':
    ...
    parser.add_argument("--lowdv", help="Resamples grid to this pixel size (km/s) when passed", type=float)
    
    parser.add_argument("--z-forest-min", help="Lower end of the forest. Default: %(default)s", type=float, default=1.7)
    parser.add_argument("--z-forest-max", help="Lower end of the forest. Default: %(default)s", type=float, default=3.9)
    ...
    parser.add_argument("--use_eds_v", help="Use EdS wavelength grid. Default is False (i.e. Logarithmic spacing).", \
        action="store_true")
    parser.add_argument("--ngrid", help="Number of grid points. Default is 2^20", type=int, default=2**20)
    ...
    for qso in qso_iter:
        ...
        if not REAL_DATA:
            ...
            if args.lowdv:
                ...
                wave, fluxes, errors = max_obs_spectrum.wave, max_obs_spectrum.flux.reshape(1,max_obs_spectrum.N), \
        ...
        if args.chunk_dyn:
            ...
            temp_fname = ["%s_%s_%s-%d_%dA_%dA%s.dat" % (qso.qso_name, max_obs_spectrum.pi_date, \
                max_obs_spectrum.spec_prefix, nc, wave[nc][0], wave[nc][-1], settings_txt) for nc in range(nchunks)]
        else:
            wave  = [wave]
