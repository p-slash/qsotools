...
def main():
    ...
    parser.add_argument("--plotfname", "-pf",
    ...
    print(f"Minimum pixel spacing is {minmin_dv:.2f} km/s"
          f", which corresponds to k_Nyquist={k_nyq:.2e} s/km")
    print(f"Mean between spectra {meanmin_dv:.2f} km/s"
          f", which corresponds to <k_Nyq>={meanmin_dv:.2e} s/km")
    ...
    if config_qmle.k_edges[-1] > k_nyq:
        print(
            f"{bcolors.BOLD}Warning: The last k edge is above the Nyquist.{bcolors.ENDC}")
        if config_qmle.k_edges[-2] > k_nyq:
            print(
                f"{bcolors.FAIL}{bcolors.BOLD}Error: The last k bin is empty!{bcolors.ENDC}")
        else:
            print("But last bin is not empy.")
    else:
        print(
            f"{bcolors.OKBLUE}{bcolors.BOLD}Passed: Ks end before the Nyquist frequency.{bcolors.ENDC}")
    ...
    if config_qmle.sq_vlength < maxmax_dv:
        ...
        print(
            f"{bcolors.BOLD}Increase VelocityLength to at least {maxmax_dv:.1f}.{bcolors.ENDC}")
