...
class GetNCopy(object):
    def __init__(self, args):
        ...
        if self.args.remove_targetid_list:
            self.ids_to_remove = np.loadtxt(
                self.args.remove_targetid_list, dtype=int)
        else:
    ...
    def _calc_mean_reso(self, hdr):
        if not self.args.calc_mean_reso:
            return
    ...
    def getFlistFromOne(self, f):
        ...
        for hdu in fts[1:]:
            ...
            slst.add(self._calc_mean_reso(hdr))
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "--snr-cut", help="S/N cut using MEANSNR in header.",
        default=0, type=float)
    parser.add_argument(
        "--remove-targetid-list",
        help="txt file with targetid to exclude from final list.")
    parser.add_argument("--calc-mean-reso", action="store_true")
    ...
    if all_slst:
        temp_fname = ospath_join(args.Directory, f"specres_list.txt")
    ...
    if args.snr_cut <= 0 or not all_low_flst:
        temp_fname = ospath_join(args.Directory, f"fname_list.txt")
        print("Saving spectra file list as ", temp_fname)
        saveListByLine(all_high_flst, temp_fname)
    else:
        temp_fname = ospath_join(
            args.Directory, f"fname_list-high-snr{args.snr_cut}.txt")
        print("Saving high snr spectra file list as ", temp_fname)
        saveListByLine(all_high_flst, temp_fname)

        temp_fname = ospath_join(
            args.Directory, f"fname_list-low-snr{args.snr_cut}.txt")
