...
def one_jackknife_est(chunk, total_power, total_fisher):
...
def main():
    ...
    pcounter = Progress(len(args_list))
    for chunks, power, fisher in imap_it:
    ...
    args_list = [(chunk, total_power, total_fisher) for chunk in all_chunks]
    all_powers = pool.starmap(one_jackknife_est, args_list)
    logging.info("Done.")
    pool.close()
