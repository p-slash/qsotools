...
def main():
    ...
    parser.add_argument(
        ...
        type=int, default=70)
    parser.add_argument(
        "--deltaz", help="Redshift bin width. Default: %(default)s",
        type=float, default=0.05)

    parser.add_argument(
        "--ngrid", help="Number of grid points. Default is 2^18",
        type=int, default=2**18)
    parser.add_argument(
        "--griddv",
        help="Pixel size of the grid in km/s. Default: %(default)s",
    ...
    k_centers, power_sp_z = lm.lognPowerSpGH(
        redshifts, args.ngrid, args.griddv)
    ...
    aln_power_table = Table(
        [zarr_repeated, bin_centers_repeated, power_sp_z.ravel()],
        names=('z', 'kc', 'P-ALN'),
        meta={
            'comments': [
                "Analytic expression computed using following parameters:",
                "Lognormal Mocks", f"{number_of_z_bins} {number_of_k_bins}"]}
    )
    aln_power_table.write(
        fname_power + ".tbl",
        format='ascii.fixed_width',
        formats={'z': '%.4f', 'kc': '%e', 'P-ALN': '%e'},
        overwrite=True)
