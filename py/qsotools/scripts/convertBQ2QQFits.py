...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "filelist", help="List of files. Starts with number of files.")
    parser.add_argument("OutputDir")
    parser.add_argument("--relative-dir", default=".",
                        help="Relative directory to fnames in filelist.")
    parser.add_argument(
        "--nside", help="Heal pixel nside. Default: %(default)s",
        type=int, default=8)
    parser.add_argument(
        "--read-coords",
        help="Reads coords from file. If not randomly generated.")
    parser.add_argument("--seed", help="Default: %(default)s",
                        default=3434672, type=int)
    ...
    for ipix in range(npixels):
        ...
        fname = ospath_join(
            dir2, f"lya-transmission-{args.nside}-{ipix}.fits.gz")
