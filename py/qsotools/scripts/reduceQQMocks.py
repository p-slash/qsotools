...
def getForestAnalysisRegion(wave, z_qso, args):
    ...
    w1 = max(fid.LYA_WAVELENGTH * (1 + args.z_forest_min),
             args.desi_w1, fid.LYA_FIRST_WVL * (1 + z_qso))
    w2 = min(fid.LYA_WAVELENGTH * (1 + args.z_forest_max),
             args.desi_w2, fid.LYA_LAST_WVL * (1 + z_qso))
...
class Reducer(object):
    def openFITSFiles(self, fname):
        def rreplace(s, new):
            return new.join(s.rsplit("/spectra-", 1))
        ...
        if self.args.output_dir != self.args.Directory:
            ...
            self.fname_delta = ospath_join(
                self.args.output_dir, self.fname_delta)

        if not (self.args.nosave or self.args.compute_mean_flux):
            self.fitsfiles['Delta'] = fitsio.FITS(
                self.fname_delta, "rw", clobber=True)
    ...
    def forEachArm(self, arm, fbrmap):
        ...
        nspectra = self.fitsfiles['Spec'][f'{arm}_FLUX'].read_header()[
            'NAXIS2']
        ARM_FLUXES = self.fitsfiles['Spec'][f'{arm}_FLUX'].read()
        ARM_IVAR = self.fitsfiles['Spec'][f'{arm}_IVAR'].read()
        ARM_MASK = np.array(
            self.fitsfiles['Spec'][f'{arm}_MASK'].read(), dtype=bool)
        ...
        for i in range(nspectra):
            ...
            def isShort(x):
                return self.args.skip and (np.sum(x) < MAX_NO_PIXELS * self.args.skip)
            ...
            if self.args.oversample_rmat > 1:
                try:
                    rmat = so.getOversampledRMat(
                        rmat, self.args.oversample_rmat)
                except Exception as e:
                    logging.error(e)
                    logging.error(
                        "Oversampling failed. TARGETID: %d, Npix: %d.", thid, wave.size)
    ...
    def __init__(self, args):
        ...
        self.local_meanflux_hist = so.MeanFluxHist(
            args.z_forest_min, args.z_forest_max)

    def __call__(self, fname):
        self.openFITSFiles(fname)
        self.fname = fname
        fbrmap = self.fitsfiles['Spec']['FIBERMAP']['TARGETID',
                                                    'TARGET_RA', 'TARGET_DEC'].read()
...
def main():
    ...
    parser.add_argument(
        "Directory", help="Directory. Saves next to spectra-X-X.fits as delta-X-X.fits")
    parser.add_argument(
        "--output-dir", help="Save results here if passed.", required=True)
    ...
    if args.P_folders is None:
        args.P_folders = [int(x) for x in next(
            os_walk(args.Directory))[1] if x.isdigit()]
        logging.info(
            "Transversing all P folders. There are %d many.", len(args.P_folders))
    else:
        # args.P_folders = [x for x in args.P_folders if x.isdigit()]
        logging.info(
            "Transversing given P folders. There are %d many.", len(args.P_folders))
