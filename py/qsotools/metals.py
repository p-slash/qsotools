...
class AbsorptionProfile(object):
    ...
    def getRelativeFluxStrengths(ion, log10N, b):
        ...
        for i in range(ntransitions):
            ...
            nu12 = fid.LIGHT_SPEED * 1e13 / l12
            tau = AbsorptionProfile._getOpticalDepth(nu12, l12, log10N, b, transitions[i].f_osc)
    ...
    def getEquivalentWidth(ion, log10N, b, unit='A'):
        ...
        for transition in IonTransitions[ion]:
            ...
            def oneminusflux(l):
                nu = fid.LIGHT_SPEED * 1e13 / l
                return 1-np.exp(-AbsorptionProfile._getOpticalDepth(nu, l12, log10N, b, f12))

            ew = quad(oneminusflux, l12-100, l12+100, epsabs=1e-8, epsrel=1e-08, limit=500)[0]
